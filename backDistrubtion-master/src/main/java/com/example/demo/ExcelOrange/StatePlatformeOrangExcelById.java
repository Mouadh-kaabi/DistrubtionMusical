...
public class StatePlatformeOrangExcelById {
...
private XSSFWorkbook workbook ;
	...
	public StatePlatformeOrangExcelById(List<OrangeExcel> listDetailsArtistes)
	{
	...
	private void writeHeaderLine()
	{
		...
		style.setFont(font);
		createCell(row,0,"Nom Platforme",style);
		
		createCell(row,1,"Nombre d'ecoute",style);
		//createCell(row,2,"Revenu Golbal Operation",style);
		createCell(row,2,"Tax Telecome",style);
		createCell(row,3,"Total Net de redvance",style);
		createCell(row,4,"Revenu Golbal Smart",style);
	
		createCell(row,5,"Htva",style);
	...
	private void writeDataLine ()
	{
		...
		for(OrangeExcel det : detailsArtistes)
		{
			...
			createCell(row,columnCount++,det.getPlateforme(), style);
